...
- **Observatorio v1 (ADR-012): CONSTRUIDO, DESPLEGADO Y VERIFICADO EN VIVO** (rebanadas 1 y 2, 2026-07-15, bitácora abajo): parser Reducciones ERV + agregados + página de vertimiento + snapshot de la demo + Action + Pages. **Sitio: https://faborubio.github.io/acopia/** (index 200 con "2,463 GWh"; `demo.html` 200). Enlazado en el README (2026-07-16).
- **Observatorio rebanada 3 (enmienda ADR-012.2): PUBLICADA EN VIVO** (2026-07-15c, bitácora abajo): duck curve del CMg + valorización del desplazamiento a la punta, **verificada en https://faborubio.github.io/acopia/** ("La duck curve del costo marginal" + "US$ 137 M" + "CMg 2025 (referencia)" presentes; run de Pages `success`). El bloqueo ("fuente de CMg automatizable") quedó obsoleto tras ADR-012.1 → la fuente es el caché `datos/cmg/` (hoy: S.GREGORIO 2025, ~300 KB) en el mismo ritual mensual local. Duck curve real: colapso a 29.7 USD/MWh a mediodía, punta 113 a las 22:00; **US$ 137 M** para los 2.463 GWh ene–may 2026 (η 85%, referencia declarada). Queda ampliar a las 3 barras con CMg 2026 en el refresco de agosto.
- **PRÓXIMA ACCIÓN:** ver `## Próxima sesión` de `CLAUDE.md` (lista canónica renumerada 1–8). Titulares: ⏸ difundir el Observatorio (perfil de GitHub + LinkedIn, lo ejecuta el autor — ahora con la valorización US$ 137 M como titular); refresco mensual (~agosto: ERV de junio + XLS de CMg 2026 de las 3 barras representativas → reemplazar la referencia 2025 en el workflow); prospección Maule con el Observatorio como artefacto de venta. La postulación a Junior Data Engineer (Grupo Mariposa) sigue en curso; la oferta de pulir el dashboard para el CV queda abierta. ✅ Key SIP rotada (2026-07-09). ✅ Carpeta renombrada `ergia`→`acopia` (2026-07-11). ✅ Sweep de ventana + dashboard demo (2026-07-12).
...
### 2026-07-15c — Observatorio rebanada 3: duck curve del CMg + valorización a la punta (enmienda ADR-012.2)
- **El hallazgo que desbloqueó la rebanada:** el bloqueo declarado ("falta fuente de CMg automatizable para el Action") se escribió ANTES del pivote de ADR-012.1; desde esa enmienda el Action no descarga nada, así que la descarga manual del XLS de CMg es tan "automatizable" como los ERV. **Enmienda ADR-012.2 (SAD → v1.3.2):** caché versionado `datos/cmg/` en el mismo ritual mensual local; API v4 descartada con evidencia doble (ignora filtro de barra + 429, y Cloudflare bloquea CI). **Decisiones del autor:** caché (no API) y **2–3 barras representativas** (norte = S.GREGORIO, continuidad; centro tipo Alto Jahuel; sur tipo Charrúa — mnemónicos exactos al bajar los XLS).
- **Agregados puros** (`observatorio/agregados.py`): `duck_curve_usd_mwh` (promedio horario, exige cobertura de las 24 h — hora sin datos = serie mal leída, error) y `valor_desplazamiento_usd` = `Σ_h E_h · max(0, η·CMg_punta − CMg_h)` (la regla de honestidad del ADR-012: diferencial contra la punta, no spot ni punta a secas; η validada en (0,1]).
- **Sitio:** sección duck curve (línea SVG server-side, hasta 3 barras) + sección valorización (tarjeta por barra: US$ M, hora punta, η y **año de referencia del CMg declarado**); la nota metodológica al pie explica la fórmula cuando hay CMg y conserva la promesa "página futura" cuando no. Paleta = **slots 5–7 de la referencia** (aqua/naranjo/violeta — una barra nunca se disfraza de tecnología), **validada con el script dataviz en ambos modos** (PASS; el WARN del aqua en claro se releva con etiquetas directas + tooltip). Sin `--cmg` la página es la v1 exacta (retrocompatible, tests viejos intactos).
- **CLI:** `observatorio --cmg "ETIQUETA=COLUMNA=RUTA"` (repetible; misma etiqueta concatena archivos; convenciones fijas del XLSX ancho: Fecha/Hora, escala 1000) + `--eficiencia` (default 0.85). `.gitignore` de datos ganó la excepción `cmg/`; el workflow ganó el trigger `datos/cmg/**` y el ritual CMg documentado en el encabezado (los flags `--cmg` concretos se agregan cuando exista el caché).
- **Smoke con datos reales** (S.GREGORIO 2025 completo, los 3 XLSX en disco): duck curve real — colapso a **29.7 USD/MWh** a mediodía, punta **113 a las 22:00** — y valorización **US$ 137 M** para los 2.463 GWh de ene–may 2026. Revisado a ojo en ambos temas (Edge headless; fix de la pasada: la unidad "USD/MWh" del eje quedaba recortada anclada a la derecha → anclada al borde izquierdo).
- **Deuda nueva AUD-028:** la valorización usa la curva promedio de todo el período de referencia (sin calce mes a mes, y 2025 vs vertimiento 2026 hasta agosto) — declarado en el sitio; el calce mensual es la mejora natural cuando el caché tenga 2026.
- **PUBLICADA EN VIVO en la misma sesión (decisión del autor: no esperar agosto):** los 3 XLS de S.GREGORIO 2025 se copiaron a `datos/cmg/` (`sgregorio_2025_{ene,feb,mar-dic}.xlsx`, ~300 KB) y el paso de build del workflow ganó los `--cmg`; run de Pages `success` y verificación en vivo (sección + US$ 137 M + "CMg 2025 (referencia)"). En agosto: reemplazar/añadir los XLS 2026 de las 3 barras y actualizar los flags.
- (El correo "Some jobs were not successful" que recibió el autor era el run viejo de las 22:04 UTC — el `workflow_dispatch` del debugging pre-ADR-012.1 que murió con el 403 de Cloudflare; falsa alarma, todo posterior en verde.)
- Verde: ruff OK · mypy 102 OK · import-linter 2 KEPT · pytest **233 passed** (+9) · pip-audit limpio.
...
- **Despliegue verificado en vivo:** run de Pages `success`; `https://faborubio.github.io/acopia/` responde 200 con la página real (2,463 GWh, enero–mayo 2026) y `demo.html` 200 con el snapshot. README de acopia enlaza el sitio (header + sección demo).
- Gotchas de la sesión: PowerShell here-string (`@'...'@`) NO es sintaxis de Git Bash (commit con título basura → amend + force-with-lease); el guión largo tipográfico (U+2013) en tests se escribe con su escape `\u2013` (RUF001 lo caza literal); `dict` es invariante en mypy → `Mapping` para parámetros; `10 ** int` es `Any` para mypy → `10.0 **`; `ws.cell(..., value=None)` de openpyxl es no-op.
