...
- **PRÓXIMA ACCIÓN:** el usuario está **postulando a Junior Data Engineer (Grupo Mariposa, remoto, aplicación en español)** — el dashboard `GET /demo` (ADR-011) es la vitrina para esa postulación y quedó abierta la oferta de **iterarlo/pulirlo** (textos, KPIs, modo screenshot para el CV) si el usuario lo pide. Sin eso: ninguna fase pendiente (Fase 5 = "solo con tracción"); dominios/INAPI diferidas a propósito (abajo); deuda viva del registro (tras el sweep, lo vivo de AUD-005 son hiperparámetros/regla por régimen; AUD-006 backtest de política con LSTM es candidata natural). ✅ Key SIP rotada (2026-07-09). ✅ Carpeta renombrada `ergia`→`acopia` (2026-07-11). ✅ Sweep de ventana + dashboard demo (2026-07-12, bitácora abajo).
