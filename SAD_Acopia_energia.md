...
> Estructura: arc42 · **Versión: 1.3.2** (historial al final) · Estado: alcance de portafolio completo — fases 0–4 cerradas (sign-offs en [`docs/AUDIT.md`](./docs/AUDIT.md)); fase 5 solo con tracción; en curso: Observatorio (ADR-012), la cara pública
...
> **Enmienda ADR-012.1 (2026-07-15) — el Action construye desde un caché versionado, no descarga.** El plan original ("el Action descarga el XLSX nuevo") murió en el primer despliegue: coordinador.cl está tras **Cloudflare**, que desafía a las IPs de datacenter con "Just a moment..." (HTTP 403); burlar el challenge desde CI no es una opción honesta (incidente en `docs/TROUBLESHOOTING.md`). La decisión: los XLSX públicos se versionan en **`datos/erv/`** (excepción explícita al git-ignore de `datos/`), el workflow construye el sitio **desde el repo** y se redespliega solo al pushear el caché (trigger por `paths`); el **refresco mensual corre local** — desde una IP residencial el mismo `curl` pasa sin challenge — con el snippet documentado en el encabezado del workflow. *Trade-off aceptado:* la automatización pierde el último tramo (un comando local al mes) y el repo gana ~1.8 MB/mes de XLSX (**AUD-027**: si el peso molesta, convertir el caché a un formato compacto parseado).
> **Enmienda ADR-012.2 (2026-07-15) — la fuente de CMg de la rebanada 3 (duck curve + valorización) es el mismo patrón de caché, no la API.** El bloqueo declarado ("falta fuente de CMg automatizable para el Action") quedó obsoleto tras ADR-012.1: como el Action ya no descarga nada, la descarga manual del XLS de CMg del Coordinador es tan "automatizable" como los ERV — se suma al **mismo ritual mensual local** y se versiona en **`datos/cmg/`**. La vía API v4 se descarta con evidencia doble: ignora el filtro de barra en el servidor (~150.000 registros/día de todas las barras + 429, `docs/TROUBLESHOOTING.md`) y Cloudflare bloquea al runner igual que bloqueó los ERV. *Alcance:* duck curve con **tres barras representativas** (norte = S.GREGORIO____013, ya usada en F2–F4; centro y sur = barras de referencia tipo Alto Jahuel y Charrúa, mnemónicos exactos al momento de la primera descarga — el matching tolerante de la ingesta los absorbe). *Valorización del desplazamiento a la punta* (regla de honestidad del ADR-012): diferencial contra la hora punta — `Σ_h E_vertida_h × max(0, η·CMg_punta − CMg_h)` — con **eficiencia de ida y vuelta η declarada** en el sitio, reportada como **rango entre barras** mientras AUD-026 (sin mapeo central→zona) siga viva. *Transición:* el desarrollo parte con el CMg 2025 de S.GREGORIO ya en disco (etiquetado como referencia); los meses 2026 de las tres barras entran al caché en el refresco de agosto. *Peso:* ~27 KB/barra·mes — AUD-027 casi no se agrava.
...
| 1.3.1 | 2026-07-15 | **Enmienda ADR-012.1**: el Action construye desde el caché versionado `datos/erv/` (Cloudflare bloquea la descarga desde CI con challenge JS); refresco mensual local + redespliegue por push. Deuda nueva AUD-027 (peso del caché XLSX). |
| 1.3.2 | 2026-07-15 | **Enmienda ADR-012.2**: la fuente de CMg de la rebanada 3 es el caché versionado `datos/cmg/` (mismo ritual local de 012.1; API v4 descartada con evidencia); duck curve con 3 barras representativas (S.GREGORIO + centro + sur); valorización a la punta por diferencial con η declarada, reportada como rango entre barras (AUD-026 sigue viva). |
